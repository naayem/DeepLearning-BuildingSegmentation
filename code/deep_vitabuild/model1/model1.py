...
def model1(dataset_dir, cropped_img_dir, opening_info_dir, floor_dir, detec_cfg, building_metadata):
    ...
    for file_address in tqdm(dataset):
        ...
        roof_slope=0
        list_floor_info, list_num_floor, list_roof_slope, list_floor_slope, list_lon_lat = [], [], [], [], []
        for i in df_building.building_index:
          if not np.isnan(i):
            floor_img =  predicted_image
            slope_threshold_1, slope_threshold_2, roof_slope = handle_slope_threshold(im, i, masks, boxes, range_threshold, r2_socre_threshold, degree_close =25, degree_far=15, flag=True)
            floor_img, list_floor_centroid, num_floor, floor_slope, Longitude, Latitude = run_plot_whole(file_address, floor_img, df_building, dict_centroid, boxes, predicted_image,  i, delta_x_threshold, slope_threshold_1, slope_threshold_2, col = 'y' )
            ...
            list_roof_slope.append(roof_slope)
            list_floor_slope.append(floor_slope)
            list_lon_lat.append((Longitude,Latitude))
        ...
        df_building['roof_slope'] = pd.Series(list_roof_slope)
        df_building['average_slope'] = pd.Series(list_floor_slope)
        df_building['gps_coordinates'] = pd.Series(list_lon_lat)
        #for l in range(0,len(list_floor_slope)-1):
        #    if (list_floor_slope[l]!=0 and list_roof_slope[l]!=0 and abs(list_floor_slope[l]-list_roof_slope[l])/max(list_floor_slope[l],list_roof_slope[l])>=20/100):
        #        print(str(indexs_building[l])+"dif = "+str(abs(list_floor_slope[l]-list_roof_slope[l])/max(list_floor_slope[l],list_roof_slope[l])))
        if len(list_floor_slope)>1:
            for l in list_floor_slope:
                print(l)
