...
def get_building_centroid(df_building, building_index, dict_centroid):
    ...
    cur_df_centroid = df_centroid[df_centroid.label.isin(cur_list_labels)]  # DataFrame
...
# get slope for the right side of the opening
...
# get slope for the left side of the opening
...
# find the centroids that form the same floor 
def find_group_member(df_left_corner, df, group, bounding_box_y_range, delta_x_threshold, slope_threshold_1, slope_threshold_2):
    ...
    while added_label != -1 and added_label != None:
        ...
        added_label = find_slope_right(df_added_label, df_closet_sorted_right, group, x_0, y_0, bounding_box_y_range, delta_x_threshold, slope_threshold_1, slope_threshold_2)
    ...
    while added_label != -1 and added_label != None:
        ...
        added_label = find_slope_left(df_added_label, df_closet_sorted_left, group, x_0, y_0, bounding_box_y_range, delta_x_threshold, slope_threshold_1, slope_threshold_2)

# create a list containing all the centroids of the same floor
def create_group(df, bounding_box_y_range, delta_x_threshold, slope_threshold_1, slope_threshold_2):
    ...
    find_group_member(df_left_corner, df, group, bounding_box_y_range, delta_x_threshold, slope_threshold_1, slope_threshold_2)
    return group

# combine all groupes in one list
def find_group_list(df, bounding_box_y_range, delta_x_threshold, slope_threshold_1, slope_threshold_2):
    group_list = []
    while df.shape[0] != 0:
        group = create_group(df, bounding_box_y_range, delta_x_threshold, slope_threshold_1, slope_threshold_2)
        group_list.append(group)
        df = df[~df.label.isin(group)]
    return group_list

# drow the floor line on the image and give the mean slope of the floors
def draw_line(group_list, cropped_img, cur_df_centroid, col='y'):
    x_array, y_array = [], []
    list_of_floor_slopes= []
    mean_floor_slope_per_building = 0
    for group_labels in group_list:
        ...
        if len(x_array) !=0 and c.size>1 :
          best_fit_slope, best_fit_intercept = np.polyfit(x_array, y_array, 1)
          list_of_floor_slopes.append(best_fit_slope)
    if len(list_of_floor_slopes)!=0:
      mean_floor_slope_per_building = np.mean(list_of_floor_slopes)
    return cropped_img, mean_floor_slope_per_building

# Execute all steps of the floor detection
def run_plot_whole(file_address, out_img, df_building, dict_centroid, boxes, predicted_image, building_index, delta_x_threshold, slope_threshold_1, slope_threshold_2, col='y'):
    cropped_img, bounding_box_min, bounding_box_y_range = crop_image( boxes, building_index,predicted_image)
    cur_df_centroid = get_building_centroid( df_building, building_index, dict_centroid)
    group_list = find_group_list(cur_df_centroid, bounding_box_y_range, delta_x_threshold, slope_threshold_1, slope_threshold_2)
    final_image, floor_slope = draw_line(group_list, out_img, cur_df_centroid, col='y')
    #build_plot(group_list,file_address,cur_df_centroid,'/content/semester-project/code/data/JS.json', col='y')
    Longitude, Latitude = get_gps_coordinates('/content/semester-project/code/data/JS.json',file_address)
    list_floor_centroid = find_floor_centroid(group_list, cur_df_centroid)
    num_floor = len(list_floor_centroid)
    return final_image, list_floor_centroid, num_floor, floor_slope, Longitude, Latitude
...
# get the coordinates of the points that for the roof and the ground of the building detected
def find_roof_bottom_points(origin_image, building_index, masks, boxes, range_threshold):
    ...
    roof_mean_height = df_roof.mean(axis=0)['y']
    bottom_mean_height = df_bottom.mean(axis=1)
    return df_roof, df_bottom, roof_mean_height, bottom_mean_height
...
def find_roof_slope(origin_image, building_index, masks, boxes, range_threshold=0.6, r2_socre_threshold=0.8, show_figure=False):
    ...
    df_roof, df_bottom, roof_height, bottom_height= find_roof_bottom_points(origin_image, building_index, masks, boxes, range_threshold)
    ...
    return degree, -slope, score, roof_height

def find_bottom_slope(origin_image, building_index, masks, boxes, range_threshold=0.6, r2_socre_threshold=0.8, show_figure=False):
    df_roof, df_bottom, roof_height, bottom_height = find_roof_bottom_points(origin_image, building_index, masks, boxes, range_threshold)
    slope_bottom, score_bottom = linear_simulate(df_bottom, show_figure)
    if score_bottom >= r2_socre_threshold:
        slope, score = slope_bottom, score_bottom
        degree = degrees(atan(-slope))
    else:
        degree, slope, score = np.nan, np.nan, np.nan
    return degree, -slope, score, bottom_height

# second method for the floor detection
# unused here
def interpolate_slopes(origin_image, building_index, masks, boxes, cur_df_centroid):
    group_list = []
    x,roof_slope,y,roof_height = find_roof_slope(origin_image, building_index, masks, boxes)
    x,bottom_slope,y,bottom_height = find_bottom_slope(origin_image, building_index, masks, boxes)
    for index1, row1 in cur_df_centroid.iterrows():
      group = []
      for index2, row2 in cur_df_centroid.iterrows():
        if index1 == index2 or any(index1 in sublist for sublist in group_list) or any(index2 in sublist for sublist in group_list):
          continue
        group.append(index1)
        s = slope(row1['x'],row1['y'],row2['x'],row2['y'])
        y = (row1['y']+ row2['y'])/2
        expected_slope = roof_slope *((y-bottom_height)/(roof_height-bottom_height)) +bottom_slope*((roof_height-y)/(roof_height-bottom_height))
        min_expected = expected_slope*0.95
        max_expected = expected_slope*1.05
        if s>min_expected and s<max_expected:
          group.append(index2)
      group_list.append(group)
    return group_list
...
#calculate the threshold for the angles based on the roof slope
def handle_slope_threshold(origin_image, building_index,  masks, boxes, range_threshold, r2_socre_threshold, degree_close=25, degree_far=15, flag=False):
    ...
    return slope_threshold_1, slope_threshold_2, res[1]
