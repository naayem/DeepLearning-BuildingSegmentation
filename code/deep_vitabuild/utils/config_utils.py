...
def get_config(exp_config):
    config_filename = ''
    if isinstance(exp_config, str): #check if valid string argument
        config_filename = exp_config
        with open(exp_config) as f: #Open the yaml
            exp_config = edict(yaml.load(f, Loader=yaml.FullLoader)) #load yaml as edict
