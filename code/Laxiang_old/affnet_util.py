...
# ============ Affnet  ============
# more detail please look at weblsite below
# https://github.com/ducha-aiki/affnet/blob/master/examples/SIFT-AffNet-HardNet-kornia-matching.ipynb
...
def detect_DoG_AffNet_OriNet_HardNet(img, weight_path, nfeats = 5000, dev=torch.device('cpu')):
    hardnet = KF.HardNet(True).to(dev).eval()

    affnet = torch.jit.load(f'{weight_path}/AffNetJIT.pt').to(dev).eval()
    affnet.eval()

    orinet = torch.jit.load(f'{weight_path}/OriNetJIT.pt').to(dev).eval()
    orinet.eval()
...
def ransac_validate(tentatives, kpts1, kpts2, inl_th = 1.0):
    ...
    inliers_num = deepcopy(mask).astype(np.float32).sum()
    # print(inliers_num, 'inliers found')
    return H, mask, inliers_num

def decolorize(img):
    return  cv2.cvtColor(cv2.cvtColor(img,cv2.COLOR_RGB2GRAY), cv2.COLOR_GRAY2RGB)

def draw_matches(kps1, kps2, As1, As2, tentatives, inl_mask, img1, img2, H_gt, H):
